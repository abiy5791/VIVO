...
function DashboardCard11() {
  ...
  const fetchSubmittedTasks = async (system_coordinator_id) => {
    try {
      const res = await axios.get(
        `systemCoordinators/${system_coordinator_id}/submitted_tasks/`
      );
  ...
  useEffect(() => {
    if (system_coordinator_id) {
      fetchSubmittedTasks(system_coordinator_id);
    }
