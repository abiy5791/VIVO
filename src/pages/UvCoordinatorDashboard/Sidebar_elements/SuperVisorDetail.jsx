...
const SuperVisorDetail = () => {
  ...
  console.log(item);
  const [user, setUser] = useState(item || {}); // Initialize user with item
  ...
  if (!item) return <div>Loading...</div>; // Optional: Handle case where item is not available yet
    ...
    <div className="flex">
      <div className="flex px-3 py-3">
        <div className="max-w-sm rounded overflow-hidden shadow-lg">
          <img
            className="w-full"
            src={user.image || item.avatar} // Use user image if updated, else use initial item image
            alt="User"
          />
          {isEditing && (
            <div>
            ...
            {[
              ...
              >
                <dt className="text-sm font-medium text-gray-500">
                  {field === "first_name" && "Full name"}
                  {field === "email" && "Email address"}
                  {field === "phone_number" && "Phone number"}
                  {field === "department" && "Department"}
                  {field === "specialization" && "Specialiazation Field"}
