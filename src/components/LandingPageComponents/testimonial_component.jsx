export default () => {
  ...
  return (
    <section className="relative py-8 mb-24">
      <hr class="mx-auto my-8 h-1 w-1/3 border-0 bg-gray-300" />
        ...
        <div className="mt-12">
          <ul className="grid items-center gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {testimonials.map((item, idx) => (
              <li
                key={idx}
                className="bg-white rounded-xl border shadow-md dark:bg-transparent"
              >
                <div className="p-4">
                  <svg
                    className="w-9 h-9 text-gray-300 dark:text-slate-400 "
                    viewBox="0 0 35 35"
                    fill="none"
                ...
                <figure>
                  ...
                  <div className="flex items-center gap-x-4 p-4 mt-6">
                    <img
                      src={item.avatar}
                      className="w-16 h-16 rounded-full border-2 border-indigo-500 dark:border-transparent "
                    />
                    <div>
                      <span className="block text-gray-800 font-semibold dark:text-slate-100 ">
                        {item.name}
                      </span>
                      <span className="block text-indigo-600 text-sm mt-0.5 dark:text-indigo-400 ">
                        {item.title}
                      </span>
                    </div>
                  </div>
