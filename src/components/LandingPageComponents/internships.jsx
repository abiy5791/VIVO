...
function Internships() {
  const [InternshipPost, setInternshipPost] = useState([]);

  useEffect(() => {
    axios
      .get("/posts/?type=Internship")
      .then((response) => {
        setInternshipPost(response.data);
      })
      .catch((error) => {
          ...
          {InternshipPost.length > 0 ? (
            ...
            <div>
              ...
              <h1 className="font-extralight text-xl mt-3 text-center">
                No Internship Post data is available !
              </h1>
            </div>
          )}
        </div>
